...
const menuItems = [
  ...
  { name: 'Skills', icon: ChartBarIcon },
  { name: 'Portfolio', icon: FolderIcon },
  { name: 'Certificates', icon: CheckBadgeIcon },
...
<template>
  <aside class="md:col-start-1 md:row-start-1 md:row-span-2 bg-white rounded-2xl shadow-sm p-6 flex flex-col items-center relative overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col items-center w-full h-full">
      ...
      <div class="relative w-36 h-36 mb-4">
        ...
        <img
          src="/me.jpeg"
          alt="Foto profil"
          class="relative w-36 h-36 rounded-full object-cover border-4 border-white shadow"
        />
      </div>

      <h1 class="text-lg font-bold text-gray-900">Chaerul Fajar Subhi</h1>
      <p class="text-sm text-orange-500 font-medium mb-6">Web Programmer & Graphic Designer</p>

      <nav class="w-full space-y-1 mb-6">
        <button
          v-for="item in menuItems"
          :key="item.name"
          @click="emit('update:activeMenu', item.name)"
          class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition"
          :class="props.activeMenu === item.name
      ...
      <div class="flex gap-3 mt-auto">
        ...
        <a href="#" class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition">
          <svg class="w-5 h-5 text-gray-700" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            ...
            <path stroke-linecap="round" stroke-linejoin="round" d="M3.6 9h16.8M3.6 15h16.8M12 3a15 15 0 0 1 4 9 15 15 0 0 1-4 9 15 15 0 0 1-4-9 15 15 0 0 1 4-9Z"/>
          </svg>
        </a>
      </div>
